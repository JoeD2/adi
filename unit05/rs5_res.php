...
$password = $_SESSION['counter5_rs'];
$username_err = $password_err = "";
$unit_num = 5;
$page_num = 7;
$username = "{$_SESSION["username"]}{$unit_num}_{$page_num}_rs_res";
...
$last_answered = 1;
while ($check <= 6) {
    $sql = "SELECT password FROM users WHERE username = '{$_SESSION["username"]}{$unit_num}_{$check}_rs'";
...
if($result = mysqli_query($link, $sql)){
    if(mysqli_num_rows($result) > 0){
        ...
        // Free result set
        mysqli_free_result($result);
    } else {$password = $_SESSION['counter5_rs'];}
} else{
    echo "ERROR: Could not able to execute $sql. " . mysqli_error($link);
}
